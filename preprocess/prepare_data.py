    ...
    for na in names:
        ...
        if recompute or (not os.path.isfile(out_path)):
            ...
            if audio.shape[0] == 0:
                ...
                # Dump to pickle
                pickle.dump(x, open(out_path, 'wb'), protocol=4)
        cnt += 1
...
def files_list(path):
    filelist = pd.read_csv(path,header=0)
    ls = filelist['filepath'].tolist()
    idlist = filelist['id'].tolist()
    namelist = filelist['name'].tolist()
    return ls,idlist,namelist

### Harmonic-Percussive Source Separation Feature extraction. (by Evan Yang)
def extract_hpss_features(wav_dir, out_dir, state, recompute):
    ...
    create_folder(out_dir)
    audio_list,id_list = files_list(wav_dir)
    # names = [na for na in os.listdir(wav_dir) if na.endswith(".wav")]
    # names = sorted(names)
    print("Total file number: %d" % len(audio_list))
    # states = [["train",cfg.train_len],["test",cfg.test_len]]
    cnt = 0
    t1 = time.time()
    import pdb;pdb.set_trace()
    for na in range(2500):
        wav_path = audio_list[na]
        out_path = out_dir + '/' + str(id_list[na]) + '_data'+ "_" + str(na % 100) + '.p'
...
### Load data directly from files instead of csv file
def extract_hpss_features_without_silence(wav_dir, out_dir, recompute):
    """Extract Harmonic-Percussive Source Separation features.
    ...
    for na in names:
        ...
        if recompute or (not os.path.isfile(out_path)):
            print(cnt, out_path)
            (audio, _) = read_audio(wav_path, fs)
            # audio = remove_dc_and_dither(audio,fs)
            # Skip corrupted wavs
            if audio.shape[0] == 0:
                print("File %s is corrupted!" % wav_path)
            else:
...
def extract_hpss_features_pthread(wav_dir,out_dir,state,recompute,start,end,mode):
    """Extract Harmonic-Percussive Source Separation features.
    ...
    if not os.path.isdir(out_dir):
        create_folder(out_dir)
    audio_list,id_list,name_list = files_list(wav_dir)
    # names = [na for na in os.listdir(wav_dir) if na.endswith(".wav")]
    # names = sorted(names)
    print("Total file number: %d" % len(audio_list))
    # states = [["train",cfg.train_len],["test",cfg.test_len]]
    ...
    for na in range(start,end):
        wav_path = audio_list[na]
        # out_path = out_dir + '/' + str(id_list[na]) + "_" + str(na % states[state][1]) + "_" + states[state][0] + '.p'
        out_path = out_dir + '/' + name_list[na] + '.p'
        #　out_path = out_dir + '/' + str(id_list[na]) + '_data'+ "_" + str(na % 100) + '.p'
        print(str(cnt)+"/"+str(end-start), out_path)
        # Skip features already computed
        if recompute or (not os.path.isfile(out_path)):
            (audio, _) = read_audio(wav_path, fs)
            print("Extracting........")
            ...
            if audio.shape[0] == 0:
                ...
                if mode == 2:
                    ...
                    mel_spec = np.stack([mel, mel_harmonic, mel_percussive], axis=0)
                
                x = mel_spec.astype(np.float64)
                print(x.shape)
                # Dump to pickle
                pickle.dump(x, open(out_path, 'wb'), protocol=4)
        cnt += 1
...
def get_spectrogram(wav):
    D = librosa.stft(wav, n_fft=1023,hop_length=360,win_length=1023, window='hamming')
    spect, phase = librosa.magphase(D)
    print(spect.shape)
    return spect

def extract_hpss_features_sg_pthread(wav_dir,out_dir,state,recompute,start,end, mode, window_length = 512,window_shift=64):
    """Extract Harmonic-Percussive Source Separation features.
        ...
        if recompute or (not os.path.isfile(out_path)):
            ...
            if audio.shape[0] == 0:
                ...
                if mode == 2 or mode == 3:
                    ...
                    spec_h = np.log(get_spectrogram(y_harmonic) + 1e-8)
                    # spec_h = (spec_h-np.mean(spec_h))/np.std(spec_h)
                    spec_h = spec_h.T
                    spec_h = pad_trunc_seq(spec_h,max_len)
                    
                    spec_p = np.log(get_spectrogram(y_percussive) + 1e-8)
                    # spec_p = (spec_p-np.mean(spec_p))/np.std(spec_p)
                    spec_p = spec_p.T
                    spec_p = pad_trunc_seq(spec_p,max_len)
                
                if mode == 1 or mode == 3:
                    spec = np.log(get_spectrogram(audio) + 1e-8)
                    # spec = (spec - np.mean(spec))/np.std(spec)
                    spec = spec.T
                    spec = pad_trunc_seq(spec,max_len)

                if mode == 2:
                    spec = np.stack([spec_h, spec_p], axis=2)
                elif mode == 3:
                    spec = np.stack([spec,spec_h, spec_p], axis=2)
                
                x = spec.astype(np.float64)
                print(spec.shape)
                # Dump to pickle
                pickle.dump(x, open(out_path, 'wb'), protocol=4)
        cnt += 1
...
def pack_features_to_hdf5(intent, fe_dir, out_path):
    ...
    with h5py.File(out_path, 'w') as hf:
        hf.create_dataset('na_list', data=n_na_all)
...
if __name__ == '__main__':
    # extract_hpss_features_without_silence("data/nosilence/train","data/extraction/train",False)
    # extract_hpss_features_without_silence("data/nosilence/test","data/extraction/test",False)
    extract_hpss_features("data.csv","data/extraction",0,False)
